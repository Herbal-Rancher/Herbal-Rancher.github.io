---
layout: page
permalink: /network-portfolio/test-prep/
nav_exclude: true
---

---
---
---


# Test Prep

This is my central test-preparation workspace.

Each exam area is organized into its own study path with:

- A prep home page
- A targeted review
- An interactive test engine
- Focused drills
- Missed-question retesting
- Full practice simulations

The goal is to make each study track repeatable:

**Review → Test → Identify Weakness → Retest → Repeat**


<div style="height: 3px; background: #f4b400; margin: 30px 0;"></div>

---
---
---

## Available Test Prep

<div class="prep-grid">

  <div class="prep-card">
    <p class="prep-label">AVAILABLE NOW</p>
    <h2>SF 1041 Networks Core</h2>
    <p>
      Targeted preparation for the City and County of San Francisco
      Technology Engineer - Associate (1041), Networks Specialty exam.
    </p>

    <p><strong>Focus areas:</strong></p>

    <ul>
      <li>Routing and EIGRP</li>
      <li>Switching and VLANs</li>
      <li>Firewalls and network security</li>
      <li>802.1X and secure wireless</li>
      <li>Network services and monitoring</li>
      <li>Troubleshooting and analytical thinking</li>
      <li>Multifunction devices and documentation</li>
    </ul>

    <p>
      <a class="prep-button" href="/network-portfolio/sf-1041-network-exam-prep/">
        Open SF 1041 Prep →
      </a>
    </p>
  </div>

  <div class="prep-card prep-card-future">
    <p class="prep-label">PLANNED</p>
    <h2>CompTIA Network+</h2>
    <p>Dedicated Network+ review, labs, domain drills, and exam simulations.</p>
  </div>

  <div class="prep-card prep-card-future">
    <p class="prep-label">PLANNED</p>
    <h2>CompTIA Security+</h2>
    <p>Dedicated Security+ review, security scenarios, domain drills, and exam simulations.</p>
  </div>

</div>

---

## How I Use This Site

I begin at this index, choose the exam I am preparing for, and then work through that exam's own study path.

For each exam:

1. Open the **Prep Home**.
2. Review the highest-priority topics.
3. Run focused questions for weak areas.
4. Retest missed questions.
5. Complete a full simulation.
6. Return to review only where the results show a weakness.

This keeps each certification or employment exam separate while allowing one consistent study method across all of them.

<style>
.prep-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(260px, 1fr));
  gap: 1rem;
  margin: 1.25rem 0 2rem;
}

.prep-card {
  border: 1px solid rgba(127,127,127,.35);
  border-radius: 14px;
  padding: 1.15rem;
}

.prep-card-future {
  opacity: .68;
}

.prep-label {
  font-size: .75rem;
  letter-spacing: .08em;
  font-weight: 700;
  opacity: .72;
}

.prep-button {
  display: inline-block;
  margin-top: .5rem;
  padding: .65rem .9rem;
  border: 1px solid currentColor;
  border-radius: 8px;
  font-weight: 700;
  text-decoration: none;
}
</style>

---
---
---

## 🔗 Navigation

* [Home](/)
* [Network Portfolio](/network-portfolio/)
  * [Formative Modules](/network-portfolio/formative-modules/)
  * [Video Walkthroughs](/network-portfolio/videos/)
  * [Study Diagrams](/network-portfolio/study-diagrams/)
  * **[TEST PREP](/network-portfolio/test-prep/)**
* [Trading+](/trading/)
* [Bible Study](/bible-study/)
* [About the Portfolio](/about/)

---
---
---